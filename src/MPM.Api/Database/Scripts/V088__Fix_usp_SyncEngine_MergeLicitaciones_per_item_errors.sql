-- El backfill 2025-2026 mostro que un solo item con datos invalidos (ej. un CodigoEstado que
-- no esta en estados_licitacion, distinto de los 5 verificados en V086: Publicada/Cerrada/
-- Desierta/Adjudicada/Revocada) hace que el EXCEPTION WHEN OTHERS de nivel de procedimiento
-- aborte TODO el lote del dia (varios cientos de licitaciones perdidas por un solo item malo).
-- Se mueve el manejo de errores a nivel de item: un item invalido se cuenta como error y se
-- salta, sin perder el resto del lote.
...
BEGIN
    ...
    LOOP
        ...
        EXCEPTION WHEN OTHERS THEN
            -- Un item invalido (ej. codigo_estado no catalogado) no debe perder el resto del
            -- lote del dia; se registra en p_error_msg (el ultimo error visto) y se continua.

-- V084 corrigio la firma del procedimiento pero dejo un bug de tipos: licitaciones.raw_data
-- es JSONB, y v_item->>'raw_data' devuelve TEXT (el operador ->> siempre extrae como texto).
-- Postgres no tiene cast implicito de TEXT a JSONB, asi que el INSERT fallaba con
-- "column raw_data is of type jsonb but expression is of type text". Se agrega el cast
-- explicito en el INSERT y en el UPDATE (via EXCLUDED, que hereda el tipo de la columna
-- insertada, asi que solo hace falta corregir el VALUES).

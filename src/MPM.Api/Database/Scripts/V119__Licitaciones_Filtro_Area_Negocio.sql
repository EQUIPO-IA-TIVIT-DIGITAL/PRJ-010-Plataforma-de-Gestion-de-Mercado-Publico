-- V119: US1 (spec 031) — filtro de licitaciones por área de negocio.
-- Agrega p_area/p_sin_clasificar a usp_Licitaciones_Listar (base: V093), reutilizando
-- fn_licitacion_area_codigos (V118). Nuevos parámetros al final con default NULL.
--
-- IMPORTANTE (corregido en V125 tras un fallo real detectado por el test suite completo):
-- agregar parámetros con CREATE OR REPLACE FUNCTION crea un OVERLOAD nuevo, no reemplaza
-- la función existente -- Postgres exige DROP FUNCTION primero cuando cambia la firma de
-- entrada, exactamente el mismo patrón que V079 ya usaba al agregar p_sort_by/p_sort_dir
-- sobre la versión de 8 parámetros (ver ese archivo). Esta migración (V119) se dejó tal
-- cual para no reescribir historia ya aplicada en entornos existentes -- el DROP real
-- vive en V125.
...
BEGIN
    ...
    -- area tiene prioridad sobre sin_clasificar si vienen ambos (contracts/licitaciones-area-y-estadisticas.md)

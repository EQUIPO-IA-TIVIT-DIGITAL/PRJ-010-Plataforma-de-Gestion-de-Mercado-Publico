-- V135: Fix de usp_Admin_ListarLogs (V132/V134) — tipos text vs VARCHAR(20).
-- plpgsql valida que la estructura del RETURN QUERY coincida EXACTAMENTE con el
-- RETURNS TABLE. Los literales del primer SELECT del UNION ('auth', 'exito') y el
-- CASE del bloque ai_provider se resuelven como text, y text no es asignable a
-- VARCHAR(20) en esa verificación → 42804 "Returned type text does not match
-- expected type character varying in column 2/4". Fix: casts explícitos.
...
BEGIN
    ...
    SELECT e.id AS id, 'auth'::VARCHAR(20) AS tipo, e.created_at::TIMESTAMP AS fecha,
           'exito'::VARCHAR(20) AS estado,
    ...
    SELECT s.id AS id, 'sync'::VARCHAR(20) AS tipo, s.ejecutado_en AS fecha, s.estado::VARCHAR(20) AS estado,
    ...
    SELECT sc.id AS id, 'scraper'::VARCHAR(20) AS tipo, sc.ejecutado_en AS fecha, sc.estado::VARCHAR(20) AS estado,
    ...
    SELECT x.id AS id, 'extraccion'::VARCHAR(20) AS tipo, x.ejecutado_en AS fecha, x.estado::VARCHAR(20) AS estado,
    ...
    SELECT a.id AS id, 'ai_provider'::VARCHAR(20) AS tipo, a.updated_at::TIMESTAMP AS fecha,
           (CASE WHEN a.record_status = 'A' THEN 'activo' ELSE 'historial' END)::VARCHAR(20) AS estado,

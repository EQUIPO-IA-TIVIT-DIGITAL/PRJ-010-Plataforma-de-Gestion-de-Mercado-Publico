-- V153: Habilitar extensión unaccent y wrapper para búsqueda acento-insensible
-- Problema: websearch_to_tsquery es sensible a acentos ("anonimizacion" != "anonimización")
-- Solución: función IMMUTABLE unaccent_immutable + índice expresión + SP usa expresión índice

-- 1. Habilitar extensión unaccent (requiere superuser, ejecutar una sola vez)
CREATE EXTENSION IF NOT EXISTS unaccent;

-- 2. Función wrapper IMMUTABLE para usar en índices
CREATE OR REPLACE FUNCTION unaccent_immutable(text) RETURNS text
LANGUAGE sql IMMUTABLE PARALLEL SAFE
AS $$ SELECT unaccent($1) $$;

-- 3. Recrear índice con unaccent_immutable en todos los campos textuales
DROP INDEX IF EXISTS idx_licitaciones_search_vector;

CREATE INDEX idx_licitaciones_search_vector
ON licitaciones
USING gin (
    (setweight(to_tsvector('spanish', unaccent_immutable(coalesce(nombre,''))), 'A') ||
     setweight(to_tsvector('spanish', unaccent_immutable(coalesce(descripcion,''))), 'B') ||
     setweight(to_tsvector('spanish', unaccent_immutable(coalesce(codigo_externo,''))), 'C'))
)
WHERE deleted_at IS NULL;

-- 4. Actualizar SP usp_Licitaciones_Listar para usar índice de expresión con unaccent_immutable
DROP FUNCTION IF EXISTS usp_Licitaciones_Listar(INT, INT, VARCHAR, SMALLINT, VARCHAR, VARCHAR, DATE, DATE, VARCHAR, VARCHAR, SMALLINT, BOOLEAN, DECIMAL, DECIMAL);
...
CREATE OR REPLACE FUNCTION usp_Licitaciones_Listar(
    ...
    p_monto_desde DECIMAL DEFAULT NULL,
    p_monto_hasta DECIMAL DEFAULT NULL
)
RETURNS TABLE (
    Id BIGINT,
...
BEGIN
    ...
    IF p_search IS NOT NULL AND length(trim(p_search)) > 0 THEN
        -- unaccent_immutable en la query para búsqueda acento-insensible
        v_query := websearch_to_tsquery('spanish', unaccent_immutable(p_search));
    END IF;

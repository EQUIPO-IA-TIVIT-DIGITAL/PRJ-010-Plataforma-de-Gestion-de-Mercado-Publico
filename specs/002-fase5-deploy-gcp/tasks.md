...
- [x] T005 [P] **Hecho 2026-07-07**: instancia Memorystore `redis-cu010` (`REDIS_7_2`) provisionada en `vpc-cu010`. ⚠️ Tier `BASIC` (sin failover) — el correo de Nicolás decía "capacidad estándar", verificar si es intencional o si hace falta pedir `STANDARD_HA`.
- [x] T006 [P] **Hecho 2026-07-06, parcialmente cerrado 2026-07-07**: `mpm-api-sa` y `mpm-jobs-sa` creadas, con `roles/storage.objectAdmin` scopeado al bucket. Nicolás otorgó `roles/cloudsql.client` y `roles/secretmanager.secretAccessor` a ambas, y `roles/run.invoker` a `mpm-jobs-sa` — verificado vía `gcloud projects get-iam-policy`. ⚠️ **Sigue faltando `roles/aiplatform.user` en ambas SA** (necesario para Gemini/Vertex AI, `020-migracion-gemini-adc` — sin él, el análisis de documentos responde 403 en producción). Pendiente de re-solicitar a Nicolás.
...
**Checkpoint**: infraestructura de red/seguridad lista; recién aquí tiene sentido tocar código.

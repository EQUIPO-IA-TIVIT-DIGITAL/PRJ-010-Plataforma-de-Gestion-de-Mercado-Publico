...
### ✅ P0 — Cerrado 2026-08-04

| Feature | Spec | Cubre el pedido # | Estado |
|---|---|---|---|
| Filtro por área de negocio, estadísticas de estado con drill-down, orden de análisis por fecha de adjudicación, informe ejecutivo de competidores con actividad total de mercado, flujo colaborativo go/no-go | [`031-feedback-chilecompra`](031-feedback-chilecompra/spec.md) | nuevo (reunión "[PRJ-010] ChileCompra - Feedback", 2026-08-03) | **Implementado y cerrado 2026-08-04**, validado en vivo contra Docker real (build + test suite completo, escenarios de `quickstart.md` corridos contra el stack real). Las 5 User Stories completas: US1 (filtro por área), US3 (orden de análisis) y US2 (estadísticas con drill-down) reutilizan el `search_vector` ya existente sin infraestructura nueva; US5 (flujo colaborativo) es un módulo nuevo (`MPM.Modules.Colaboracion`) que reutiliza tablas de Mensajería, validado E2E con Playwright; US4 (actividad de mercado de competidores) fue la única que requirió investigación en vivo (Claude in Chrome, credenciales autorizadas) — encontró y usó un endpoint HTTP público de Mercado Público más liviano que Playwright para la búsqueda, corrida real completa confirmó que el riesgo de volumen documentado en el research no se materializó. 2 bugs reales encontrados y corregidos durante la validación (ver `CHANGELOG.md`: overload SQL duplicado en `usp_Licitaciones_Listar`, y un campo `id` nunca mapeado en `LicitacionHandler`). La evaluación de despliegue on-premise mencionada en la misma reunión quedó fuera de esta spec ("Future Considerations") — se sigue tratando por separado (benchmark de modelos Qwen/Gemma + estimación de costos GCP). |

| Migración del proveedor de IA (Gemini → Qwen 3.7 G4) con switch de super admin (gcloud/qwen) | [`033-migracion-qwen-g4`](033-migracion-qwen-g4/spec.md) | interno (evaluación on-premise TIVIT, spec 031 "Future Considerations") | **En implementación 2026-08-11** — abstracción `ILlmClient` + resolución dinámica por request (US1), cliente OpenAI-compatible (US3), harness de benchmark (US2) y switch de super admin con endpoint + UI (US4) implementados; pendientes de entorno/negocio: ejecución real del benchmark (URL del servidor Qwen + credenciales ADC), validación manual con stack completo y cutover/rollback en staging (US5). Gemini queda como opción "gcloud" del switch y rollback — sin uso de Google a partir de la migración. |

...
- [ ] T029 Desplegar migraciones V097-V099 y el backend actualizado a producción (`tivit-cu010`), siguiendo el mismo mecanismo ya usado en `023-fix-bugs-produccion` (build local + push a Artifact Registry + `gcloud run deploy mpm-api`).
- [ ] T030 Desplegar el frontend (`mpm-web`) con el panel nuevo de Competidores y los ajustes de `AlertasPage.tsx`.
- [ ] T031 Ejecutar el scraper (`scraper-job` o localmente) sobre una muestra inicial de licitaciones adjudicadas para poblar `licitaciones_ofertas` y poder demostrar US1 con datos reales.
- [ ] T032 Re-ejecutar `quickstart.md` contra producción real para las tres historias.
- [ ] T033 Actualizar `CLAUDE.md`: marcar `024-inteligencia-competencia-alertas` con su estado real tras el deploy, actualizar la migración más alta aplicada a **V099**.

...
> ✅ **Reconciliado 2026-08-03 contra producción real** (`gcloud secrets list`, `gcloud run services describe mpm-api`, `gcloud logging read` sobre `tivit-cu010`, más lectura del código actual): el motor de alertas **está activo**. `sync-job` corre 2 veces al día (07:00/19:00 UTC) e invoca el matching en cada ciclo; Telegram tiene bot token + webhook secret cargados y el webhook `POST /api/v1/telegram/webhook` recibió llamadas reales de Telegram con `200 OK` (confirma que el flujo de deep-link de T038c quedó funcionando, no solo implementado). Se encontró y ya está corregido en git (`43c59a1`) un bug de día 1 (`reply_markup` nulo rompía el envío del resumen "Me interesa"). Quedan genuinamente pendientes: el **frontend de US2** (panel de historial, nunca se construyó) y las **unit tests de `SinonimosIaService`/`AlertaEnriquecimientoService`** (T025/T033). `CHANGELOG.md` sigue sin reflejar esta fase.
...
- [x] T035 [US5] **Verificado 2026-08-03 vía `gcloud run services describe` + `gcloud secrets list`**: `Telegram__BotToken` y `Telegram__WebhookSecret` están en Secret Manager (`telegram-bot-token`, `telegram-webhook-secret`, creados 2026-07-09) y vinculados como `secretKeyRef` al servicio `mpm-api`. `Telegram__BotUsername=CU010_bot` como env var plana. Resuelto en algún punto entre el cierre de esta sesión y el deploy — el tracker no se actualizó.
...
- [ ] T038 [P] [US5] Documentado en `quickstart.md` de la spec (no en el runbook de producción todavía) — **confirmado ausente 2026-08-03**: `docs/runbook-produccion.md` no menciona Telegram en absoluto.
- [x] T039 [P] [US5] **Verificado 2026-08-03**: `tests/MPM.Modules.Alertas.Tests/Services/TelegramNotificationServiceTests.cs` y `Controllers/TelegramWebhookControllerTests.cs` existen (escritos en algún punto posterior a esta sesión, probablemente 029).
...
- [ ] T042 [P] [US2] Frontend — **confirmado sin empezar 2026-08-03**: `AlertasPage.tsx` (231 líneas) no tiene ninguna vista de historial de disparos; el endpoint `GET /api/v1/alertas/{id}/historial` (T041) existe en backend pero no tiene UI que lo consuma.

...
`specs/024-inteligencia-competencia-alertas/plan.md`

Current features (tres en paralelo; las tres con deadline jueves 9 de julio 2026, 7:59 a.m.):

-2. **Inteligencia de competencia, alertas interactivas y canal de correo** (N-2, siguiente prioridad tras los fixes urgentes — pedidos post-demo, reunión de equipo 2026-07-09 + conversación directa con Manuel Aliaga): `specs/024-inteligencia-competencia-alertas/`. Tres historias independientes: **US1 (P1)** panel de inteligencia de competencia — busca un competidor, ve todas sus ofertas (monto + aceptada/rechazada) recolectadas del "Cuadro de Ofertas" público de Mercado Público (confirmado en vivo el 2026-07-09 que NO requiere login, ni depende de actas de evaluación como los análisis actuales de las 43 licitaciones de TIVIT), y dispara análisis de Gemini **solo bajo demanda** por rango de fechas elegido por el usuario, cacheado por competidor+periodo para no repetir el gasto. **US2 (P2)** botón inline "Me interesa" en las alertas de Telegram → resumen rápido sin Gemini, usando `ApiMpService.GetDetalleAsync` (ya existe en el código, sin usar hoy en el sync masivo) — requiere que el webhook de Telegram aprenda a manejar `callback_query` además de `/start`. **US3 (P3)** canal de alertas por correo, reusando `IEmailService`/`SmtpEmailService` ya existente (usado hoy por Auth). Spec/plan/research/data-model/quickstart generados; `tasks.md` pendiente de `/speckit-tasks`. Migraciones a crear: **V097** (`licitaciones_ofertas`), **V098** (`competidores_analisis`), **V099** (columna `email_alertas` en `alertas_destinatarios`). Riesgo abierto no bloqueante: falta un spike de muestreo para confirmar qué % de licitaciones adjudicadas realmente tiene "Cuadro de Ofertas" disponible (ver `research.md` R3).
-1. **[RESUELTO 2026-07-09] Corrección urgente de bugs detectados en producción**: `specs/023-fix-bugs-produccion/`. Dos bugs P1 encontrados en vivo contra `tivit-cu010` el mismo día del deploy inicial y de la demo de Fase 6, ninguno estaba en el QA original (`022-qa-fixes-preproduccion`) — **ambos corregidos y verificados en producción real**: **BUG-014** — crear conversación nueva en Mensajería devolvía 400 siempre. Causa real (no lo que se pensó al principio): `usp_Conversaciones_Crear` es un `PROCEDURE` invocado vía `CALL`; `ConversacionHandler.CrearAsync` mandaba `p_asunto`/`p_licitacion_id` NULL sin `dbType` explícito (viajan como `unknown`) y `p_participante_ids` sin cast a `jsonb` — Postgres no resolvía la sobrecarga (`42883`). Fix: `dbType` explícito + `::jsonb` en el SQL (`src/MPM.Modules.Mensajeria/Data/`). **BUG-015** — el auto-vinculado de Telegram en Alertas nunca marcaba `es_account_manager_gobierno=TRUE` en `alertas_destinatarios`, así que nadie que se auto-vinculara recibía alertas reales, sin error visible. Fix: `V096__Fix_Alertas_Destinatarios_Telegram.sql` (`CREATE OR REPLACE FUNCTION usp_AlertasDestinatarios_GuardarChatId` + backfill). Verificado en prod: `POST /conversaciones` → 201, `POST /alertas/{id}/probar` → `notificacionTelegramEnviada:true` real (sin el bypass manual usado durante la demo).
0. **Corrección de Hallazgos QA Pre-Producción** (N0): `specs/022-qa-fixes-preproduccion/plan.md` — 13 hallazgos de la auditoría QA técnica (Jean Franco Benique, 2026-07-07), verificados contra el código actual el 2026-07-08. **Las 9 user stories (13 bugs) están implementadas y con tests en verde** (2026-07-08), validadas en vivo contra el contenedor Docker local y contra producción real (`tivit-cu010`, deploy 2026-07-09). Migraciones **V092** (`auth_eventos`) y **V093** (fix `usp_Licitaciones_Listar`, agrega `Id BIGINT` al `RETURNS TABLE` porque V079 ya lo había extendido) — **aplicadas**.
...
>
> Nota: el feature 017 (Ajustes Urgentes del Cliente) ya fue implementado; su spec sigue en `specs/017-ajustes-urgentes-cliente/`. La migración más alta aplicada es **V096** (`V096__Fix_Alertas_Destinatarios_Telegram.sql` — fix de `usp_AlertasDestinatarios_GuardarChatId` + backfill, `023-fix-bugs-produccion`, aplicada y verificada en producción 2026-07-09) — V079 fue tomada por `003-fase6-alertas-keywords` (`usp_Alertas_*`, módulo `MPM.Modules.Alertas` ya integrado con su propio proyecto de tests); V080-V093 corrigen bugs reales del pipeline de sync/backfill de licitaciones, agregan soporte de alertas/Telegram, y corrigen los hallazgos QA de `022-qa-fixes-preproduccion` (V092 `auth_eventos`, V093 fix de búsqueda); V094/V095 son el reset + fix de secuencias de la migración masiva de datos locales a producción (2026-07-09). La siguiente libre es **V097**.
>

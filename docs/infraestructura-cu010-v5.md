...
**Área:** Digital — TIVIT  
**Fecha:** Junio 2026  
**Versión:** 5.0 — Arquitectura híbrida On-Premise + Huawei Cloud
...
|------------|-------------------|
| Backend API | .NET 8 (ASP.NET Core) — monolito modular, 9 módulos (Api, Core, Shared, Auth, Licitaciones, Catalogo, Mensajeria, Analisis, Notificaciones) |
...
| Mensajería en tiempo real | ASP.NET Core SignalR — hub `/hubs/mensajeria` |
| Almacenamiento de archivos | `LocalStorageService` — disco local del servidor en `/app/uploads` |
| Análisis IA | `GeminiService` — Google Gemini 2.5 Pro (**estado transitorio**, reemplazar en Fase 5 por Kimi 2.5) |
| Scraper | Node.js + Playwright Chromium (dentro del contenedor api, script `/tools/scraper-mp/agente-mp.js`) |
...
| **Servicio de IA** | `GeminiService` (`gemini-2.5-pro` vía Google API) — **transitorio** | `KimiService` (Kimi 2.5 vía Huawei MaaS) |
...
| Servicio Huawei | Descripción | Equivalente AWS | Uso en MPM | Rec. |
|---|---|---|---|---|
| **RDS for PostgreSQL** | PostgreSQL gestionado con backups automáticos, failover HA y réplicas de lectura | RDS PostgreSQL | Reemplazar el contenedor PostgreSQL Docker cuando el servidor on-premise migre a nube | 🔵 |
...
| **CSMS** (Cloud Secret Management Service) | Almacén de secretos gestionado. Variables sensibles fuera del sistema de archivos del servidor | Secrets Manager | Almacenar `JWT__Secret`, `AI__ApiKey`, `MP_TICKET`, `MP_PASSWORD`, `DB_PASSWORD`. Reemplaza `.env` plano en producción | ✅ |
...
> **Política**: La arquitectura objetivo usa exclusivamente servicios de IA del ecosistema Huawei Cloud. El estado actual (código con `GeminiService`) es transitorio y se migrará a Kimi 2.5 en Fase 5.

| Servicio Huawei | Descripción | Uso en MPM | Rec. |
|---|---|---|---|
| **ModelArts / MaaS** | Plataforma Huawei Cloud para acceso API a modelos LLM: Kimi 2.5, Pangu, DeepSeek R1, GLM-4 y otros del catálogo | **Servicio de IA objetivo.** Reemplaza Google Gemini. Mismo propósito: análisis de PDFs de licitaciones, extracción de criterios, puntuaciones y chat Q&A | ✅ |
| **Kimi 2.5** (vía MaaS) | Modelo LLM de Moonshot AI disponible en el catálogo Huawei MaaS. Razonamiento multimodal, análisis de documentos complejos, generación de JSON estructurado | **Modelo primario objetivo.** Análisis de documentos de evaluación (PDFs de licitaciones), extracción de criterios, puntuaciones y resúmenes ejecutivos | ✅ |
| **Pangu** (vía MaaS) | LLM propio de Huawei. Razonamiento estructurado, generación de JSON | Alternativa dentro del catálogo MaaS si Kimi 2.5 no está disponible en la región seleccionada | 🔵 |
| **DeepSeek R1** (vía MaaS) | Modelo de razonamiento de alto rendimiento disponible en el catálogo MaaS | Alternativa para bases de licitación complejas (>40 páginas, múltiples criterios) | 🔵 |
| **GLM-4** (vía MaaS) | Modelo LLM adicional en el catálogo MaaS | Contingencia ante indisponibilidad de Kimi 2.5. **Esta es la contingencia correcta para IA generativa**, no OCR. | 🔵 |
...
| **CSMS** | `JWT__Secret`, `AI__ApiKey`, `MP_TICKET`, `MP_PASSWORD`, `DB_PASSWORD` | ~USD 1–2/mes |
...
| **Huawei MaaS** (Kimi 2.5) | Análisis IA de PDFs — reemplaza Google Gemini | Variable por tokens |
...
| **Pangu / DeepSeek R1 / GLM-4** | Si Kimi 2.5 no cumple los requisitos de calidad o no está disponible en la región | LLMs alternativos en catálogo Huawei MaaS |
...
│                                                   │
│  backups/2026-06-26_backup.sql.gz   ← Standard   │
...
obsutil cp "${DUMP_FILE}" "obs://mpm-backups-bd/backups/${DATE}_backup.sql.gz" \
  -e "${STORAGE__ENDPOINT}" -ak "${STORAGE__ACCESSKEY}" -sk "${STORAGE__SECRETKEY}"
...
## 6. Estrategia de IA — Migración a Huawei MaaS (Kimi 2.5)

### Estado actual vs objetivo

| | Estado actual | Objetivo Fase 5 |
|---|---|---|
| Clase | `GeminiService.cs` | `KimiService.cs` (nueva) |
| Endpoint upload | `generativelanguage.googleapis.com/upload/v1beta/files` | No aplica — Kimi recibe texto directamente |
| Endpoint inferencia | `generativelanguage.googleapis.com/v1beta/models/gemini-2.5-pro:generateContent` | `https://maas.<region>.myhuaweicloud.com/v1/infers/moonshot-kimi-2.5` |
| Variable de entorno | `Gemini__ApiKey` | `AI__ApiKey` |
| Config key en C# | `Gemini:ApiKey` | `AI:ApiKey` |
| Modelo | `gemini-2.5-pro` | `moonshot-kimi-2.5` (validar ID en consola MaaS) |
...
└── AnalisisBackgroundService
    └── AnalisisService
        └── IAnalisisIAService
            └── KimiService (nueva implementación)
                └── POST https://maas.<region>.myhuaweicloud.com/v1/infers/moonshot-kimi-2.5
...
| Modelo | Prioridad | Fortaleza | Uso recomendado |
|---|---|---|---|
| **Kimi 2.5** (Moonshot AI) | ✅ Primario | Razonamiento multimodal, análisis documental, generación JSON estructurado | Análisis primario de PDFs de licitaciones |
| **Pangu** (Huawei) | 🔵 Alternativa | Razonamiento estructurado nativo Huawei | Si Kimi 2.5 no está disponible en la región |
| **DeepSeek R1** | 🔵 Alternativa | Razonamiento de alta calidad, documentos largos | Bases de licitación complejas (>40 páginas) |
| **GLM-4** | 🔵 Contingencia | Velocidad y costo | Clasificación rápida y resúmenes ejecutivos |
...
3. Crear `KimiService : IAnalisisIAService` con llamada a Huawei MaaS endpoint `https://maas.<region>.myhuaweicloud.com/v1/infers/moonshot-kimi-2.5`
4. En `Program.cs` agregar switch: `AI:Provider = "kimi"` → `KimiService`, `"gemini"` → `GeminiService` (rollback)
5. Reemplazar `Gemini__ApiKey` por `AI__ApiKey` en CSMS y en `docker-compose.prod.yml`
...
│                  │  │  OBS ─── PDFs licitac.  │  │  Kimi 2.5        │
│ Scraper → REST + │  │  OBS ─── Dumps BD       │  │  (Moonshot AI)   │
│ Playwright       │  │  CSMS ── Secretos       │  └──────────────────┘
└──────────────────┘  │  KMS ─── Cifrado OBS    │
                       │  WAF ─── API pública    │
                       │  LTS ─── Logs central.  │
                       │  SMN ─── Alertas email  │
...
| `generativelanguage.googleapis.com` | HTTPS / 443 | Gemini File API + inferencia (**transitorio**) | Por análisis |
...
| `maas.<region>.myhuaweicloud.com` | HTTPS / 443 | MaaS — inferencia Kimi 2.5 (`/v1/infers/moonshot-kimi-2.5`) | Sí |
...
El sistema usa **tres convenciones distintas** según el contexto:

| Convención | Formato | Aplica a | Ejemplo |
|---|---|---|---|
| **Doble guión bajo** (`__`) | `SECCION__CLAVE` | Variables en `.env` y `docker-compose.yml` — ASP.NET Core las convierte automáticamente a notación colon | `Storage__Provider=local` |
| **Colon** (`:`) | `Seccion:Clave` | Código C# — `IConfiguration`, `appsettings.json` | `Storage:Provider` |
| **Guión simple** (`_`) | `PREFIJO_CLAVE` | Variables planas del scraper Node.js — proceso separado que no usa IConfiguration | `MP_TICKET`, `SCRAPER_ENABLED` |

> Las tablas a continuación usan la convención de doble guión bajo (formato `.env` / docker-compose), que es el formato a configurar en el servidor.
...
| `JWT__Secret` | — (40+ chars) | Clave de firma JWT | Crítica |
| `JWT__Issuer` | `TIVIT.MPM` | Claim issuer del token | Alta |
| `JWT__Audience` | `MPM.Users` | Claim audience del token | Alta |
...
| `MP_DELAY_MS` | `1500` | Delay entre requests del scraper | Media |
| `MP_MAX_REINTENTOS` | `3` | Reintentos en scraper | Media |
| `MP_ANALISIS_IA` | `true` | Activar análisis IA al subir PDF | Media |
| `MP_FECHA_DESDE` | `01-01-2025` | Fecha de inicio del scraper | Media |
...
| `Gemini__ApiKey` | — | Google Gemini API key (**estado actual, transitorio — reemplazar en Fase 5**) | Crítica |
| `ASPNETCORE_ENVIRONMENT` | `Production` | Entorno de .NET | Alta |
| `Storage__Provider` | `local` | Proveedor de storage (`local` \| `obs`) | Alta |
| `Storage__LocalPath` | `/app/uploads` | Ruta de uploads locales (cuando `local`) | Alta |
...
| `Storage__Endpoint` | `https://obs.<region>.myhuaweicloud.com` | Nueva — endpoint OBS de la región |
| `Storage__AccessKey` | `<AK desde CSMS>` | Nueva — Access Key de IAM |
| `Storage__SecretKey` | `<SK desde CSMS>` | Nueva — Secret Key de IAM |
| `AI__Provider` | `kimi` | Nueva — controla qué LLM usar en tiempo de ejecución |
| `AI__Endpoint` | `https://maas.<region>.myhuaweicloud.com/v1/infers/moonshot-kimi-2.5` | Nueva — endpoint completo del modelo |
| `AI__ApiKey` | `<desde CSMS>` | Reemplaza `Gemini__ApiKey` |
| `Gemini__ApiKey` | — | Mantener en CSMS como variable de rollback hasta que Kimi 2.5 sea estable en producción |
...
8. **Kimi 2.5 — validar model-id en consola MaaS**: El identificador de modelo (`moonshot-kimi-2.5`) debe confirmarse en la consola de Huawei MaaS al momento de contratar el servicio. Huawei puede actualizar los identificadores de modelo en el catálogo. El endpoint correcto sigue el patrón `https://maas.<region>.myhuaweicloud.com/v1/infers/<model-id>` — siempre con `/v1/infers/` (número 1, no letra l).

9. **Gemini como estado transitorio**: El código actual usa `GeminiService` con Google Gemini 2.5 Pro. Este estado se mantiene solo durante el desarrollo y es incompatible con la política de exclusividad Huawei Cloud. La migración a Kimi 2.5 (vía Huawei MaaS) se implementa en Fase 5 como tarea prioritaria. Hasta que la migración sea estable en producción, `Gemini__ApiKey` se mantiene en CSMS como variable de rollback.

10. **Validación automática de scripts**: Antes de ejecutar `backup-db.sh` y `restore-db.sh` en producción, validar la sintaxis con `bash -n scripts/backup-db.sh`. Las variables `STORAGE__ENDPOINT`, `STORAGE__ACCESSKEY` y `STORAGE__SECRETKEY` deben estar disponibles en el entorno del proceso cron (configurar en `/etc/cron.d/mpm-backup` con las variables de entorno exportadas desde CSMS).

---

*Documento preparado por el equipo de Digital — TIVIT. Versión 5.0.*  
*Basado en análisis exhaustivo del código fuente (74 migraciones, 9 módulos .NET, 4 servicios Docker, 21 hooks frontend) y correcciones técnicas de gerencia.*  
*Los documentos v3 (`infraestructura-cu010.md`) y v4 (`infraestructura-cu010-v4.md`) se preservan como referencia histórica.*

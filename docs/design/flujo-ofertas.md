...
| D7.11 | **Certificaciones: la fuente correcta es `intellectual-capital/user-certifications` (verificado)** | El endpoint `services/knowledge/certifications/users` usa un catálogo chico (~20 certificaciones genéricas) — "ISO 27001" devuelve 0; "ISO" solo 3 personas. La fuente completa es `intellectual-capital/user-certifications` (8.530 registros, 5,2 MB, refrescable): el nombre canónico vive en **`certificationTypeName`** ("ISO/IEC 27001:2022 LEAD AUDITOR", "Certified ISO/IEC 27001:2022 IA / LA T-CERT", ...) con **60 registros ISO 27001** y 138 con "ISO"; cada registro trae `institutionName`, `file` (PDF de la certificación), `issuedDate`/`validDate`/`isValid`, `userName`, `corporateId`, `userId`. **Estrategia MPM**: sincronizar el dataset a la tabla local `censo_certificaciones` (refresco diario) y buscar con ILIKE/fuzzy sobre `certification_type_name` (código "27001" → 60 registros); el match de personas se cruza con technologies por `corporateId`/`userName` (el dataset no trae email); el `file` alimenta la propuesta (Fase 3) | Verificación en vivo |

...
| D2 | **Mecanismo de descarga = navegador real + sesión** | `ViewAttachment.aspx` está tras reCAPTCHA Enterprise; un `HttpClient` puro no lo resuelve (documentado en el código desde 2026-07-06). Se mantiene el camino actual (`DocumentExtractionService`/Chromium) y se conserva el extractor HTTP como fallback futuro | Verificación en vivo 2026-08-15 + código |

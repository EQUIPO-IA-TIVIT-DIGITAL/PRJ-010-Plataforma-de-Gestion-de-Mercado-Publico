...
| D7.2 | **WAF Volterra bloquea clientes de scripting (hallazgo crítico)** | El WAF (F5 Volterra, `x-volterra-location`) rechaza requests de auth desde curl/PowerShell/httpx ("Request Rejected" con Support ID) pero **permite Postman y navegadores reales** (fingerprint TLS/HTTP). Implicación: en dev local, la integración .NET con `HttpClient` directo será bloqueada → usar el **mismo patrón del scraper** (Chromium channel con fetch desde contexto de navegador) o pedir a IT allowlist de IP para dev. En prod, el WAF ya deja pasar la app | Verificación en vivo (curl vs Postman vs Chromium) |
